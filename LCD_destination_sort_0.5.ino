...
delay(5000); //Display persistence for 5 seconds
for(uint8_t i = 0; i <= Bells; i++) //Assigns destination to rounds so that after sorting to a pattern 
//the bells will always return to rounds
{
  if(Bells == 8)
  {
   destination[i] = patterns8[5][i];  //load individual bell
  //positions from pattern array into destination array
  }
  else if(Bells == 6) 
   {
    destination[i] = patterns6[5][i]; //for 6 bell patterns 
   }
   else if(Bells == 5) 
   {
    destination[i] = patterns5[5][i]; //for 5 bell patterns
   }
   else if (Bells == 4) 
   {
    destination[i] = patterns4[5][i]; //for 4 bell patterns
   }
}
